...
class InterviewService:
    ...
    def start(self, request):
        ...
        interview.questions = self.planner.create_plan(context, request.company, request.style); self.repository.save(interview)
    ...
    def answer(self, interview_id, answer):
        lock = self._mutation_lock(interview_id)
        if not lock.acquire(blocking=False):
            raise ValueError("Interview mutation already in progress")
        try:
            return self._answer_locked(interview_id, answer)
        finally:
            lock.release()

    def _answer_locked(self, interview_id, answer):
        ...
        evaluation = self.evaluator.evaluate(interview.current_question or "", answer, interview.context)
        ...
        if interview.consecutive_followups < self.MAX_CONSECUTIVE_FOLLOWUPS:
            followup = self.followups.generate(interview.current_question or "", answer, min(evaluation.technical_accuracy, evaluation.communication))
        if followup:
    ...
    def finish(self, interview_id):
        lock = self._mutation_lock(interview_id)
        if not lock.acquire(blocking=False):
            raise ValueError("Interview mutation already in progress")
        try:
            return self._finish_locked(interview_id)
        finally:
            lock.release()

    def _finish_locked(self, interview_id):
        interview = self.require(interview_id)
        ...
        report = self.reports.generate(interview.id, [x.evaluation for x in self.repository.get_evaluations(interview.id)], interview.context)
